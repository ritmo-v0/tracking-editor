...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
	return (
		<SheetPrimitive.Backdrop
			...
			className={cn(
				"fixed inset-0 bg-black/10 supports-backdrop-filter:backdrop-blur-xs transition-opacity",
	...
	return (
		<SheetPortal>
			...
			<SheetPrimitive.Popup
				...
				data-side={side}
				className={cn(
					"fixed flex flex-col bg-popover bg-clip-padding text-popover-foreground text-xs/relaxed shadow-lg",
...
function SheetTitle({ className, ...props }: SheetPrimitive.Title.Props) {
	return (
		<SheetPrimitive.Title
			...
			className={cn("font-heading font-medium text-sm text-foreground", className)}
			{...props}
		/>
	);
}
